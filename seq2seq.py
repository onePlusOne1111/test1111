...
def get_data(file):
    ...
    for line in f:
        ...
        if len(content) < 20 or len(content) > maxlen:
            continue
        dataset.append([title, content])
    f.close()
    return dataset


dataset = get_data(csv_file)

if not os.path.exists("../data"):
    os.mkdir("../data")
if os.path.exists('../data/seq2seq_config.json'):
    chars,id2char,char2id = json.load(open('../data/seq2seq_config.json'))
    ...
    json.dump([chars,id2char,char2id], open('../data/seq2seq_config.json', 'w'))


if os.path.exists("../data/random_ids.npy"):
    print("Loading random_ids from ../data/random_ids.npy.")
    random_ids = np.load("../data/random_ids.npy")
else:
    random_ids = list(range(len(dataset)))
    random.shuffle(random_ids)
    np.save("../data/random_ids.npy", random_ids)
    print("Saving random_ids to ../data/random_ids.npy")
train_data, valid_data = [], []
for id in random_ids:
    if id % 100 == 0:
        valid_data.append(dataset[id])
    else:
        train_data.append(dataset[id])
del dataset, random_ids
...
def data_generator():
    ...
    X,Y = [],[]
    while True:
        train_data_random_ids = list(range(len(train_data)))
        random.shuffle(train_data_random_ids)
        for id in train_data_random_ids:
            a = train_data[id]
            X.append(str2id(a[1]))
...
def to_one_hot(x):
    """输出一个词表大小的向量，来标记该词是否在文章出现过 若原句是"袁阳平是一个中国人",则返回的x就是在词表对应的位置是1，其它位置全是0，包括mask位置也是0
    """
    x, x_mask = x
    x = K.cast(x, 'int32')
...
def seq_maxpool(x):
    """seq是[None, seq_len, s_size]的格式，
    mask是[None, seq_len, 1]的格式，先除去mask部分，
    然后再做maxpooling。 最后的维度是[None, s_size]
    """
    seq, mask = x
    seq -= (1 - mask) * 1e10
    return K.max(seq, 1)
...
cross_entropy = K.sparse_categorical_crossentropy(y_in[:, 1:], xy[:, :-1])  # y_in和xy为什么要这么处理
...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if logs['loss'] <= self.lowest:
            ...
            model.save_weights('./best_model_0626.weights')
...
#
# with open("valid_results.csv", "w", encoding="utf-8") as writer:
#     for i, data in enumerate(valid_data):
#         title_pred = gen_sent(data[1], topk=10)
#         s = json.dumps({"origin_text": data[1], "candidate": data[0], "pred_result": title_pred},
